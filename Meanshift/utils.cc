...
void compareDataPoints() {
    ...
    for (size_t i = 0; i < dataPoints_cpu.size(); i++) {
        if (dataPoints_cpu[i].at1 != dataPoints_gpu[i].at1 ||
            dataPoints_cpu[i].at2 != dataPoints_gpu[i].at2 ||
            dataPoints_cpu[i].cluster != dataPoints_gpu[i].cluster) {
            cpugpu_same = false;
        }
        if (dataPoints_cpu[i].at1 != dataPoints_omp[i].at1 ||
            dataPoints_cpu[i].at2 != dataPoints_omp[i].at2 ||
            dataPoints_cpu[i].cluster != dataPoints_omp[i].cluster) {
            cpuomp_same = false;
        }
        if (dataPoints_cpu[i].at1 != dataPoints_mpi[i].at1 ||
            dataPoints_cpu[i].at2 != dataPoints_mpi[i].at2 ||
            dataPoints_cpu[i].cluster != dataPoints_mpi[i].cluster) {
            cpumpi_same = false;
        }
    }
